...
async function initDb() {
  ...
  const defaults = {
    'brand.name_en': process.env.BRAND_NAME_EN || 'Hamada Ahmed Ali',
    'brand.name_ar': process.env.BRAND_NAME_AR || 'حمادة أحمد علي',
    'brand.tagline_en': process.env.BRAND_TAGLINE_EN || 'Accounting, Auditing and Taxation',
    'brand.tagline_ar': process.env.BRAND_TAGLINE_AR || 'للمحاسبة والمراجعة والضرائب'
  };
  ...
  // One-time content update: carry existing installs from the old default brand
  // name/tagline to the new one. Only touches rows still holding the old text,
  // so a value someone already customized via Portal Settings is left alone.
  const renames = [
    ['brand.name_en', 'Hamada Ahmed Ali', 'Hamada Ahmed Ali for Accounting'],
    ['brand.name_ar', 'حمادة أحمد علي', 'حمادة أحمد علي للمحاسبة والمراجعة'],
    ['brand.tagline_en', 'Accounting, Auditing and Taxation', 'Accounting & Auditing — Client Documents Portal'],
    ['brand.tagline_ar', 'للمحاسبة والمراجعة والضرائب', 'المحاسبة والمراجعة — بوابة مستندات العملاء'],
  ];
  for (const [key, next, prev] of renames) {
    await pool.query(`UPDATE portal_settings SET value=$1, updated_at=NOW() WHERE key=$2 AND value=$3`, [next, key, prev]);
  }
