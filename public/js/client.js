...
(function () {
  ...
  async function refresh() {
    try {
      ...
      me = d.user; files = d.files || []; sent = d.sent || [];
      render(); renderSent();
  ...
  function render() {
    ...
    document.getElementById('stBytes').textContent = window.I18N.fmtSize(bytes);
    ...
    chips.innerHTML = folders.length
      ? [`<button class="fchip ${!folderSel ? 'active' : ''}" data-f="">${esc(t('all'))}</button>`]
          .concat(folders.map((f) => `<button class="fchip ${folderSel === f ? 'active' : ''}" data-f="${esc(f)}">${esc(f)}</button>`)).join('')
      : '';
    chips.querySelectorAll('.fchip').forEach((b) => (b.onclick = () => { folderSel = b.dataset.f === folderSel ? '' : b.dataset.f; render(); }));

    const ql = q.trim().toLowerCase();
    const shown = files.filter((f) => (!folderSel || f.folder === folderSel) && (!ql || f.name.toLowerCase().includes(ql)));
